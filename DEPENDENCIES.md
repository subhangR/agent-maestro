...
| `react-native-reanimated` | ~3.16 | UI-thread status-dot pulse, cursor blink, sheet slide, tile expand; matches Atelier's cubic-bezier eases; honors reduced-motion. | RN `Animated` (JS-driven, janks on gesture sheets + always-on pulse). |
| `react-native-gesture-handler` | ~2.20 | Required by gorhom + expo-router; gesture-driven sheets. | — |
