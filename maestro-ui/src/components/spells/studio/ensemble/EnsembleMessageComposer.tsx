...
/** Message-all composer for an ensemble (S9, FR-10.2). ⌘/Ctrl+Enter to send. */
export const EnsembleMessageComposer = React.memo(function EnsembleMessageComposer({
  ...
  return (
    <div className="spa-ens-composer" role="dialog" aria-modal="false" aria-label={`Message ${ensemble.name}`}>
      <header className="spa-ens-composer__head">
        <h5>Message all · {ensemble.memberSessionIds.length} members</h5>
      ...
      <textarea
        ...
        maxLength={4000}
        placeholder="Write a message to every member…"
        rows={4}
      ...
      />
      {error && <p className="spa-ens-composer__error" role="alert">{error}</p>}
      <div className="spa-ens-composer__actions">
        <button type="button" onClick={onClose}>Cancel</button>
        <button type="button" className="spa-ens-composer__send" disabled={!valid || sending} onClick={handleSend}>
