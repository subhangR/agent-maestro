...
describe('agent tool UI constants', () => {
  ...
  it('offers Claude Fable 5 and Sonnet 5, and tops the Claude list with Fable 5', () => {
    const claudeValues = MODELS_BY_AGENT_TOOL['claude-code'].map((model) => model.value);
    expect(claudeValues).toContain('claude-fable-5');
    expect(claudeValues).toContain('claude-fable-5[1m]');
    expect(claudeValues).toContain('claude-sonnet-5');
    expect(claudeValues[0]).toBe('claude-fable-5');
    expect(DEFAULT_MODEL_BY_AGENT_TOOL['claude-code']).toBe('claude-opus-4-8');
    expect(MODELS_BY_AGENT_TOOL.hermes.map((m) => m.value)).toContain('anthropic:claude-fable-5');
    expect(MODEL_POWER['claude-fable-5']).toBe(6.0);
    expect(MODEL_POWER['claude-fable-5[1m]']).toBe(6.1);
    expect(MODEL_POWER['claude-sonnet-5']).toBe(4.9);
  });

  it('passes active Fable 5 / Sonnet 5 model ids through unchanged (no retirement aliases)', () => {
    ...
    expect(normalizeModelId('anthropic:claude-fable-5')).toBe('anthropic:claude-fable-5');
  ...
  it('displays, ranks, and launches Fable 5 as a first-class model', () => {
    ...
    // Fable 5 outranks Sonnet 4.6 when picking the top assigned member.
