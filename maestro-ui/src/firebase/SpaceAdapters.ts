...
export async function installSpaceSpell(
  ...
  const existing = await maestroClient.getCustomPrompts();
  ...
  if (conflict) {
    ...
    if (strategy === 'replace') {
      await maestroClient.updateCustomPrompt(conflict.id, {
        name: targetName,
        content: spaceSpell.body ?? '',
        description: spaceSpell.description ?? '',
        icon: spaceSpell.icon ?? undefined,
        entityType: (spaceSpell.entityType as any) || 'session',
      });
      try {
