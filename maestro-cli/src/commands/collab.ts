...
export function pullShape(kind: 'task' | 'member' | 'spell', remote: Record<string, unknown>, projectId: string): Record<string, unknown> {
  // Local Maestro task creation has no status field. The UI creates first,
  // then applies a non-todo shared status with the update endpoint.
  if (kind === 'task') return { projectId, title: remote.title, description: remote.description || '', priority: remote.priority || 'medium', initialPrompt: remote.initialPrompt || '', dueDate: remote.dueDate || undefined, dangerousMode: remote.dangerousMode === true, useWorktree: remote.useWorktree === true };
  if (kind === 'member') return { projectId, name: remote.name, role: remote.role || '', identity: remote.identity || '', avatar: typeof remote.avatar === 'string' ? remote.avatar : '', model: remote.model || undefined, agentTool: remote.agentTool || undefined, mode: remote.mode || undefined, permissionMode: remote.permissionMode || undefined, strategy: remote.strategy || undefined, capabilities: remote.capabilities || {}, skillIds: Array.isArray(remote.skillIds) ? remote.skillIds : [], commandPermissions: remote.commandPermissions || {}, customWorkflow: remote.customWorkflow || undefined, soundInstrument: remote.soundInstrument || undefined };
  const validRules = Array.isArray(remote.rules) ? remote.rules.filter((rule) => rule && typeof rule === 'object' && typeof (rule as Record<string, unknown>).enabled === 'boolean' && typeof (rule as Record<string, unknown>).trigger === 'object' && typeof (rule as Record<string, unknown>).action === 'object') : [];
  const legacyRule = { label: 'Imported from space (legacy shared spell — review before enabling)', enabled: false, trigger: { type: 'hook', hookEvent: 'Stop' }, action: { type: 'inject-prompt', prompt: typeof remote.body === 'string' ? remote.body : typeof remote.description === 'string' ? remote.description : String(remote.name || 'Imported spell') } };
  return { name: remote.name, description: remote.description || '', icon: remote.icon || undefined, color: remote.color || 'violet', rules: validRules.length ? validRules : [legacyRule] };
}
...
function registerShareAndPull(collab: Command, root: Command, guarded: <T extends unknown[]>(action: (...args: T) => Promise<void>) => (...args: T) => Promise<void>, getClient: typeof client, getSpace: typeof requiredSpace): void {
  ...
  for (const kind of ['task', 'member', 'spell'] as const) pull.command(`${kind} <shared-id>`).requiredOption('--space <space-id>').option('--project <local-project-id>').option('--dry-run').option('--on-conflict <mode>', 'fail or copy', 'fail').action(guarded(async (id: string, options: { space: string; project?: string; dryRun?: boolean; onConflict: string }) => { if (!['fail', 'copy'].includes(options.onConflict)) throw new CollabError('INVALID_CONFLICT_MODE', 'on-conflict must be fail or copy.'); const c = await getClient(root); const collection = collectionFor(kind); const remote = await c.db.get(`collabSpaces/${options.space}/${collection}/${id}`); if (!remote) throw new CollabError('COLLAB_NOT_FOUND', 'Shared entity was not found.'); const projectId = options.project || resolveContext(c.profile, {}).projectId; if (!projectId && kind !== 'spell') throw new CollabError('COLLAB_CONTEXT_REQUIRED', 'A local project is required for pull.'); let payload = pullShape(kind, remote.data, projectId || ''); payload = await applyConflict(kind, payload, projectId || '', options.onConflict); if (options.dryRun) { result(root, { dryRun: true, kind, id, projectId, payload }); return; } const created = await api.post<Record<string, unknown>>(`/api/${kind === 'member' ? 'team-members' : `${kind}s`}`, payload); if (typeof created.id === 'string') c.db.recordPendingProvenance(collection, created.id, id, options.space); if (kind === 'task' && typeof created.id === 'string' && typeof remote.data.status === 'string' && remote.data.status !== 'todo') await api.patch(`/api/tasks/${created.id}`, { status: remote.data.status }); const fanout = kind === 'task' ? 'pulledByUids' : kind === 'member' ? 'adoptedByUids' : 'installedByUids'; try { await c.db.recordFanOut(options.space, collection, id, fanout, 'linkedLocalIdsByUid', typeof created.id === 'string' ? created.id : undefined); } catch { process.stderr.write('LOCAL_CREATED_REMOTE_AUDIT_PENDING\n'); } result(root, { kind, id: created.id, sourceId: id, projectId }); }));
  pull.command('doc <shared-id>').requiredOption('--space <space-id>').requiredOption('--out <path>').action(guarded(async (id: string, options: { space: string; out: string }) => { const c = await getClient(root); const remote = await c.db.get(`collabSpaces/${options.space}/docs/${id}`); if (!remote || typeof remote.data.content !== 'string') throw new CollabError('COLLAB_NOT_FOUND', 'Shared document was not found.'); const out = safeOutputPath(options.out); atomicWrite(out, remote.data.content); c.db.recordPendingProvenance('docs', out, id, options.space); try { await c.db.recordFanOut(options.space, 'docs', id, 'pulledByUids', 'linkedLocalIdsByUid', out); } catch { process.stderr.write('LOCAL_CREATED_REMOTE_AUDIT_PENDING\n'); } result(root, { id, out }); }));
