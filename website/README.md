...
- `index.html`, `404.html`, `styles.css`, `app.js` — the page. No framework, no tracking, no third-party scripts.
...
Two, both same-origin: none on load, and on demo-form submission `POST /api/contact`, which `firebase.json` rewrites to the `submitWebsiteInquiry` Cloud Function (`functions/src/websiteInquiry.ts`). The function validates, rate-limits, stores the enquiry in Firestore and emails a notification when `CONTACT_MAIL_PASS` is set. If the function is not deployed, `app.js` falls back to a prefilled `mailto:` draft, so the form is never a dead end.
